...
num_mess_pass_steps = [1, 2, 4, 8, 12]
num_class_steps = [1, 1, 3, 7, 11]
...
for step1, step2 in zip(num_mess_pass_steps, num_class_steps):

    model = Net({'num_enc_steps': step1, 'num_class_steps': step2}).to(device)
    LR = 0.001
    ...
    final_metrics[step1] = metrics_file


    with open(DIR_EXPERIMENT + '/message_passings.txt', 'w') as file:
        file.write(json.dumps(final_metrics))
    np.save(DIR_EXPERIMENT + '/message_passings.npy', final_metrics, allow_pickle = True)
...
plot_loss_acc_epochs_hyperparam(epoch, final_metrics, x_loss, x_acc, y_loss, y_acc, ax_acc, ax_loss, legend = {'name':'message passing steps', 'values':num_mess_pass_steps}, DIR_EXPERIMENT = DIR_EXPERIMENT)
