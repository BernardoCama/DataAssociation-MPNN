...
for name_dataset_noise in datasets_noise:
    ...
    value = values[datasets_noise.index(name_dataset_noise)]
    ...
    with open(DIR_EXPERIMENT + '/gaussian_noise.txt', 'w') as file:
        file.write(json.dumps(final_metrics))
    np.save(DIR_EXPERIMENT + '/gaussian_noise.npy', final_metrics, allow_pickle = True)
...
plot_loss_acc_values_hyperparam(final_metrics, x_loss, x_acc, y_loss, y_acc, ax_acc, ax_loss, legend = {'name':'gaussian noise', 'values':values, 'metrics': ['accuracy', 'precision', 'recall']}, DIR_EXPERIMENT = DIR_EXPERIMENT)
