...
for name_dataset_noise in datasets_noise:
    ...
    value = name_dataset_noise.split('bounding_boxes_noise_')[1].split('.mat')[0].split('_rot')[0]
    ...
    with open(DIR_EXPERIMENT + '/different_noises_rotated.txt', 'w') as file:
        file.write(json.dumps(final_metrics))
    np.save(DIR_EXPERIMENT + '/different_noises_rotated.npy', final_metrics, allow_pickle = True)
