...
for name_dataset_noise_train in []:#datasets_noise:
    ...
    for name_dataset_noise_val in datasets_noise:
        ...
        with open(DIR_EXPERIMENT + '/different_noises_validation_combinations.txt', 'w') as file:
            file.write(json.dumps(final_metrics))
        np.save(DIR_EXPERIMENT + '/different_noises_validation_combinations.npy', final_metrics, allow_pickle = True)


final_metrics = np.load(DIR_EXPERIMENT + '/different_noises_validation_combinations.npy',  allow_pickle = True).tolist()
...
values = ["Discrete", "Iso Gauss","Laplacian", "Non Iso Gauss", "Uniform"]
...
plt.savefig(DIR_EXPERIMENT + '/matrix_noises.pdf', bbox_inches='tight')
plt.savefig(DIR_EXPERIMENT + '/matrix_noises.eps', format='eps', bbox_inches='tight')
