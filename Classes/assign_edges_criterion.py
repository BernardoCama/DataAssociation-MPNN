...
def apply_heuristics (sample, output, output_fractionary):
    ...
    # Compute connected components with vehicles at nodes
    # G_components_vehicle = []
    # encoders = []
    # for i in range(len(connected_components_vehicles)):
    #     G_components_vehicle.append(nx.Graph())
    #     encoders.append(LabelEncoder().fit(connected_components_vehicles[i]))
    #     nodes = encoders[i].transform(connected_components_vehicles[i])
    #     edges = [encoders[i].transform((node_vehicle[el[0]], node_vehicle[el[1]])) for el in G_components[i].edges]
    # #     nodes, edges
    # #     connected_components_vehicles[i], [(node_vehicle[el[0]], node_vehicle[el[1]]) for el in G_components[i].edges]
    #     G_components_vehicle[i].add_nodes_from(nodes)
    #     G_components_vehicle[i].add_edges_from(edges)
    ...
    for el in connected_components_vehicles:
        ...
        #         # Remove edge with lower output from sigmoid in the connected components
        #         connected_edges_prediction = []
        #         indexes = []
        #         for edge in G_components[i].edges:
        #             index_in = np.where((edges[:, 0] == edge[0]) & (edges[:, 1] == edge[1]))[0]
        #             index_out = np.where((edges[:, 0] == edge[1]) & (edges[:, 1] == edge[0]))[0]
        #             indexes.append([index_in, index_out])
        #             #output_fractionary[index_in]
        ...
        #         index_edges_misclassified = connected_edges_prediction.index(min(connected_edges_prediction))
                ...
                pass
        i += 1
    # edgelist_zeros = edges[np.where(output==0)[0]]
